...
- 明示的な中断・再開・完了では`python3 scripts/steering_state.py --steering [日付付き名] pause / resume / complete`を使い、対象を常に明示する
- 承認が必要な場面では、選択肢を明示してユーザーの回答を待つ
- 再開依頼では最新の日付付きtasklistの`paused`状態と中断記録を読み、`--steering [日付付き名] resume --harness Kiro`で`active`へ戻してからモード2を続ける。ハーネス切り替え時はrequirements.mdの使用ハーネス欄に`Kiro`を追記する
