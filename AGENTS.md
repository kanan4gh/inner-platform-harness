...
- **実機受け入れ**: `docs/procedures/harness-acceptance.md`に従い、実行エージェントが利用許可済みのIDEまたは対話型CLIで行う。認証等で止まった場合だけ直ちにユーザーへ必要最小限の操作を依頼する。従量課金型LLM headless modeを標準受け入れに使わない
...
外部システムへアクセスするツール(MCPサーバー等)の設定形式はハーネス固有のため、各ハーネスのアダプタとプロジェクトのセキュリティ規程が定義する。
...
# 詳細レビュー
> review-docsでdocs/product-requirements.mdをレビューしてください
```

定型フローの呼び出し方はハーネスごとに異なる。

| ハーネス | 呼び出し例 |
|---|---|
| Claude Code | `/setup-project`、`/add-feature`、`/review-docs` |
| Codex | 「setup-projectを実行して」「add-featureを実行して」「review-docsを実行して」 |
| Kiro | workspace skillの`/setup-project`、`/add-feature`、`/review-docs` |
...
1. `inner-platform-harness` リポジトリを更新する
2. 各プロジェクトのAGENTS.mdの汎用層を手動で同期する
3. `UPDATED` の日付を更新して同期済みであることを記録する

### チームで使うとき

- 汎用層は `inner-platform-harness` リポジトリを正典として共有する
- プロダクト固有層・技術スタック固有層は各リポジトリで管理する
- 新メンバーは `inner-platform-harness` から汎用層をコピーしてプロジェクトに参加する
