...
SDDプロセスの正典は AGENTS.md にある(下記でインポート)。本ファイルは **Claude Code 固有の運用差分のみ**を定義するハーネスアダプタである。
...
具体的な設定境界は`.claude/README.md`とプロジェクトのセキュリティ規程で定義する。
...
- **状態遷移**: 明示的な中断・再開・完了は`python3 scripts/steering_state.py --steering [日付付き名] pause / resume / complete`で対象を明示して記録する
