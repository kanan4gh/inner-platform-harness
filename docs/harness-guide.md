# inner-platform-harness テンプレートガイド
...
- **Devcontainer環境**: Claude Code・AWS CLI・CDK・SAM CLI・GitHub CLIを利用可能
...
SDDプロセスの正典はハーネス中立の `AGENTS.md` にあり、各AIコーディングエージェント(ハーネス)固有の運用差分はアダプタが定義します。
...
- CodexまたはKiroを使う場合は、利用を許可された各製品と認証を別途セットアップしていること（このdevcontainerが導入するAIエージェントはClaude Codeのみ）
...
CodexとKiroは自動インストールされません。導入・認証・利用許可は会社の規程と各アダプタREADMEに従ってください。
...
選択したAIコーディングエージェントでsetup-projectを実行します。

| ハーネス | 呼び出し例 |
|---|---|
| Claude Code | `/setup-project` |
| Codex | 「setup-projectを実行して」 |
| Kiro | workspace skillの`/setup-project` |
...
```
1. GitHub Issue を作成
2. 利用ハーネスのadd-featureで機能実装（ステアリングファイル → 実装 → PR）
3. PR をマージ
