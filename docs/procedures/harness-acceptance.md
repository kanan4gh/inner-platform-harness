...
**不合格・保留で製品ファイルを変更した場合は、add-feature ステップ8-Bの規定に従い、影響する4段検証行と最終品質ゲート行を未完了へ戻してステップ6・7を経由し、手順1(候補ゲート)からやり直す。** 記録だけを追記して押し切らない。修正後の製品ファイルに対する実機挙動は未観察だからである。リポジトリ差分を生じさせず、環境・権限だけを解消した場合は、同じ固定commitに対してG3だけを再実施し、解消内容と再実施結果を記録する。
