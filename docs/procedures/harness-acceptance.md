...
G3は計画承認後に実行エージェントが行い、各ハーネスの操作をユーザーへ差し戻さない。既存の対話型IDEまたは対話型CLIを使って本手順を順に実行し、G3専用の自動化基盤は追加しない。

ログイン、MFA、権限確認、組織ポリシー等で続行できなくなった場合は、その場で停止して直ちにユーザーへ必要最小限の操作を依頼する。別の認証経路の調査や認証回避を続けず、解消後は実行エージェントが同じ固定commitからG3を再開する。
...
3. 製品作業と重複しない確認専用の日付付きsteering名を決め、実行エージェントがfixtureを作る。後続コマンドは辞書順の「最新」へ依存せず、その名前を常に明示する
...
6. fixture内の全チェックと振り返りを実行エージェントが完了する
